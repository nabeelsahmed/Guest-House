...
export class SharedHelpersFieldValidationsModule {
  ...
  validateToastr(validate: MyFormField[]): boolean {
    ...
    for (let i = 0; i < validate.length && this.found == true; i++) {
      if (
        ...
        } else if (validate[i].type == 'number') {
          this.found = this.validateNumber(validate[i].value, validate[i].msg);
        ...
        } else if (validate[i].type == 'SpecialChar') {
          this.found = this.validateSpecialChar(
            validate[i].value,
            validate[i].msg
          );
        } else {
  ...
  validateNumber(number: string, msg: string): boolean {
    // var regEx = /^[a-z][a-z\s]*$/;
    var regEx = /^[0-9]+$/;
